#include "include/QHexView-ng.hpp"
...
void QHexView::paintEvent(QPaintEvent *event)
{
    ...
    QByteArray data = getData(firstLineIdx * m_bytesPerLine,
            ...
            lineIdx += 1, yPos += m_charHeight)
    {
        ...
        {
            char character =
                data[(lineIdx - firstLineIdx) * (uint)m_bytesPerLine + i];
                ...
                i++, xPos += 3 * m_charWidth)
        {
            ...
            QString val = QString::number(
                              (data.at((lineIdx - firstLineIdx) * m_bytesPerLine + i) &
                               0xF0) >>
                              4,
                              16);
            ...
            val = QString::number(
                      (data.at((lineIdx - firstLineIdx) * m_bytesPerLine + i) & 0xF),
                      16);
...
void QHexView::ensureVisible()
{
    ...
    if (cursorY < firstLineIdx)
        ...
        verticalScrollBar()->setValue(cursorY -
                                      areaSize.height() / m_charHeight + 1);
}
