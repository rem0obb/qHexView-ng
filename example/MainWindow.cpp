...
void MainWindow::process(const QString &fileName)
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        QMessageBox::critical(
            this, "File opening problem",
            "Problem with open file `" + fileName + "`for reading");
        return;
    }
...
void MainWindow::slotOpen()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        settings.setValue("QHexView/PrevDir",
...
void MainWindow::slotToOffset()
{
    bool ok;
    int offset =
        QInputDialog::getInt(0, "Offset", "Offset:", 0, 0, 2147483647, 1, &ok);
